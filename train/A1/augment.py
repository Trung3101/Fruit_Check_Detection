...
def get_augment_train_kwargs() -> dict[str, Any]:
    ...
    return {
        # Throughput-first mix-based ops (reduce expensive CPU composites)
        "mosaic": 0.2,
        "mixup": 0.1,
        "copy_paste": 0.0,
        "close_mosaic": 12,
...
def build_albumentations_pipeline(img_size: int = 640) -> Any:
    """
    High-throughput Albumentations pipeline.
    Exactly 2-3 transforms are sampled uniformly from the pool per image.
    """
...
def get_fast_external_augment_pipeline(img_size: int = 640) -> tuple[str, Any]:
    """
    Returns the external augmentation backend.
    Albumentations only.
    """
    if _module_available("albumentations"):
        return "albumentations", build_albumentations_pipeline(img_size=img_size)
    raise ImportError("albumentations is not installed")
